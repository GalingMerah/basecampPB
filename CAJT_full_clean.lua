return function()
    ...
    local function InitializeMainScript()
        ...
        do
            -- Bagian ini telah dihapus karena tidak lagi memiliki akses ke gameStatusData
            HubTabs.GameStatus:Section({ Title = "Current Game" })
            HubTabs.GameStatus:Paragraph({
                Title = "Game data not available",
                Desc = "This script was not provided with game status information."
            })
        ...
        do
            ...
            GameTabs.SpaceEvent:Toggle({
                ...
                Value = false,
                Callback = function(value)
                    autoFarmState.AutoClaimSpaceAFK = value
                    if value and player then
                        task.spawn(function()
                            local spaceEventRemote = replicatedStorage:WaitForChild("Msg") and replicatedStorage.Msg:WaitForChild("RemoteFunction")
                            if not spaceEventRemote then
                                WindUI:Notify({ Title = "Error", Content = "RemoteEvent for Space Event not found.", Duration = 5, Icon = "alert-triangle" })
                                autoFarmState.AutoClaimSpaceAFK = false
                                return
                            end
                            while autoFarmState.AutoClaimSpaceAFK and player do
                                pcall(function()
                                    spaceEventRemote:InvokeServer("ClaimSpaceAFKReward")
                                end)
                                task.wait(30)
                            end
                        end)
            ...
            GameTabs.SpaceEvent:Button({
                Title = "Teleport to Space Event",
                Callback = function()
                    if msgRemote then
                        local args = { "TeleportToTargetWorld", 13 }
                        msgRemote:FireServer(unpack(args))
                        WindUI:Notify({ Title = "Teleporting...", Content = "Attempting to teleport to Space Event...", Duration = 3, Icon = "send" })
                    else
                        WindUI:Notify({ Title = "Error", Content = "Teleport remote not found.", Duration = 4, Icon = "alert-triangle" })
                    end
                end
